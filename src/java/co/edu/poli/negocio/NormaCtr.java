...
public class NormaCtr {
    ...
    public boolean desactivar(norma norm) throws SQLException{
         Connection con = Conexion.getConexion();
//        System.out.println(norm);
        PreparedStatement st = con.prepareStatement(NormaSQL.Desactivar());
        st.setInt(1, estados.INACTIVO.getValor_estado());
        st.setInt(2, norm.getId_norma());
        int r = st.executeUpdate();
        if(r == 1)
            return true;
        else
            throw new SQLException("El código de la norma no existe.");
    }

    public norma ConsultarNorma(norma norm) throws SQLException {
        Connection con = Conexion.getConexion();
//        System.out.println(norm);
        PreparedStatement st = con.prepareStatement(NormaSQL.ValidarNormaSQL());
        st.setString(1, norm.getCodigo_norma());
        ...
        if(r.next()) {
            ...
            norm.setCodigo_norma(r.getString("codigo_norma"));
            norm.setDescripcion(r.getString("descripcion"));
            norm.setEstado(r.getInt("estado"));
            System.out.println(norm);
            return norm;
        } else {
             throw new SQLException("Este código de norma " + norm.getCodigo_norma() + " no existe.");
        }
    }
}
